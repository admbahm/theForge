...
*   **Atomic Job and Packet Writes**: Job-note changes use a same-directory temporary file (`.filename.*.tmp`), sync, and atomic rename. Application packets are staged and synced as complete directories, include a deterministic manifest, and replace prior packets with rollback protection.
